...
- 教师登录、学案备课、学案发布、开课、签到、课堂实时雷达、求助查看、作业批改、AI 草稿、班级数据分析、教学反思
- 教师 / 学生资源中心支持上传、下载、分类筛选、分类治理与下载计数
- 管理员通过教师入口登录，在同一教师控制台中使用迁移兼容、学期设置、教师账号管理、教师授课班级分配、学生导入与学生编班能力
- 管理台支持资源分类治理、治理审计日志、本地 SQLite 备份快照与恢复入口
- 第二所学校演示数据与跨校治理上下文切换
...
- 本地开发默认通过 Vite 同源代理访问 `/api`，代理目标是 `http://127.0.0.1:8001`
- 如果你手动修改过 `frontend/.env`，需要重启前端 dev server 才会重新读取环境变量
...
- 后端：`LEARNSITE_SQLITE_URL`、`LEARNSITE_RESOURCE_STORAGE_DIR`、`LEARNSITE_BACKUP_STORAGE_DIR`、`LEARNSITE_CORS_ORIGINS`
...
```powershell
cd backend
python scripts/resource_center_smoke.py
```
...
- 支持学生花名册检索、课堂现场状态查看与一键定位到学生作品
- 支持班级数据分析与教学反思沉淀
- 支持学案草稿保存、发布与撤回发布
- 支持按教师授课班级过滤开课范围，新教师无历史课次时可直接进入空态控制台发起第一节课
- 支持资源上传、按分类治理、按受众/分类/状态筛选与下载计数查看
...
- 支持学年学期设置与当前学期切换
- 支持资源分类创建、启停用与教师上传联动
- 支持教师账号创建、资料维护、密码重置、启停用，以及教师可授课班级分配；平台管理员可维护学校管理员态账号
- 支持学生批量导入（粘贴 CSV / 表格文本）
- 支持学生名册查看、班级重新分配、密码重置与启停用
- 支持治理审计日志查看、本地 SQLite 备份快照创建与恢复入口
